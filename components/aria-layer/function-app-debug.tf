...
resource "azurerm_linux_function_app" "example1" {
  ...
  storage_account_name       = azurerm_storage_account.example1[each.key].name               #data.azurerm_storage_account.xcutting[each.value.lz_key].name                                     
  storage_account_access_key = azurerm_storage_account.example1[each.key].primary_access_key #data.azurerm_storage_account.xcutting[each.value.lz_key].primary_access_key 
  virtual_network_subnet_id  = data.azurerm_subnet.lz["ingest${each.value.lz_key}-data-product-001-${var.env}"].id

  app_settings = {
    APPLICATIONINSIGHTS_CONNECTION_STRING                 = azurerm_application_insights.example1[each.key].connection_string
    AzureWebJobsFeatureFlags                              = "EnableWorkerIndexing"
    BUILD_FLAGS                                           = "UseExpressBuild"
    ENABLE_ORYX_BUILD                                     = true
    ENVIRONMENT                                           = var.env
