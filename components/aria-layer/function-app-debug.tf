...
resource "azurerm_linux_function_app" "test11" {
  ...
  storage_account_name       = azurerm_storage_account.example["00"].name               #data.azurerm_storage_account.xcutting[each.key].name
  storage_account_access_key = azurerm_storage_account.example["00"].primary_access_key #data.azurerm_storage_account.xcutting[each.key].primary_access_key
