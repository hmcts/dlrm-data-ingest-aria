...
resource "azurerm_key_vault_secret" "curated_sas_token" {
  ...
  value = "BlobEndpoint=${data.azurerm_storage_account.curated[each.key].primary_blob_endpoint};SharedAccessSignature=${azurerm_storage_account_sas.curated[each.key].sas}"
