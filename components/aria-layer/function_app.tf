...
resource "azurerm_linux_function_app" "example" {
  ...
  storage_account_name       = azurerm_storage_account.xcutting1[each.value.lz_key].name               #azurerm_storage_account.example[each.key].name               
  storage_account_access_key = azurerm_storage_account.xcutting1[each.value.lz_key].primary_access_key #azurerm_storage_account.example[each.key].primary_access_key 
  ...
  app_settings = {
    ...
    AzureWebJobsStorage                   = azurerm_storage_account.xcutting1[each.value.lz_key].primary_connection_string
  ...
  depends_on = [
    azurerm_storage_account.xcutting1
  ]
}
