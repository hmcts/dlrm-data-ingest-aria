...
provider "databricks" {
  alias                       = "stg-00"
  azure_workspace_resource_id = data.azurerm_databricks_workspace.db_ws["stg-00"].id #what if we move the provider into the .tfvars? or into the folder path where stg is
